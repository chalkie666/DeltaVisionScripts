...
of user defined n by n dims, and user defined separation between fields of view.  
...
# use optparse for command line option parsing,
# to define input file and output file names and other parameters 
...
def main():
   ...
   # list of valid patterns that we have makeXxxxPattern functions for to make such patterns of panels
   validPatterns = ["Square", "Cross"]
   inputParams = getInputParams(validPatterns)

   # open and read the input .pts file 
   originalPointsFile = open(inputParams['infile'])
   originalPoints = originalPointsFile.readlines()
   #print(originalPoints)
   
   # decide what pattern of panels to use determined by -p command line parameter
   # check for valid ones we have a pattern generation funtion for and complain if not.   
   # now check the string entered as -p PATTERN command line argument is one we have a method for.
   if inputParams['pattern'] in validPatterns:  
      thePatternName = inputParams['pattern']
      patternFunctionName = "make"+thePatternName+"Panels"
      print("pattern name is allowed so pattern function name is: ", patternFunctionName)
      panelPattern = eval(patternFunctionName)(inputParams['rowsandcolumns'], inputParams['separation'])
   else:
      print("something really bad happened, bacuse invalid pattern names should be caught in getInputParams()")
      return
...
def getInputParams(validPatterns):                        
   ...
   commandLineOptionParser.add_option("-p", "--pattern",  
                                      ...
                                      help="pattern for panels must start with a Capital Letter. \
                                           It can be, for example, Square or Cross, etc.", metavar="PATTERN") 
   ...
   if commandLineOptions.pattern not in validPatterns:
       commandLineOptionParser.error("You must give a valid command line option:\
          -p PATTERN, eg. Cross, Square, etc. Must start with a Capital letter!")
...
upright cross of 2x2 has 4 panels like square grid rotataed, but 3x3 isnt, as it has only 5 fields. 
...
def makeCrossPanels(rowsandcolumns, separation):
   ...
   if int(rowsandcolumns) % 2 == 1:  # if it's odd
      ...
      print("vertical panels and horizontal panels ", listOfSeparatedVerticalPanels, listOfSeparatedHorizontalPanels)
...
"""
        ...
        panel position calc maths
        for a square matrix of panels, not very hip: 
...
def renumberNewPoints(allNewPoints):
   ...
   allRenumberedNewPoints = list(allNewPoints)
   return allRenumberedNewPoints



# now we have defined all the functions, run the main function "t' akchully dee summat"!
main()
